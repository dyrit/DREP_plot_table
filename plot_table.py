...
if task=='plot1':
    ...
    for data in plot_list2:
        plt.plot(data[:,1],data[:,2],'o',mec=line_sty[cl],mfc='none',markersize=10,label = f'{data[0,0]:.2f}g/cc')
        cl+=1
    ...
    plt.legend(bbox_to_anchor=(0.5, 1.2),loc='upper center',fontsize=20,fancybox=True, framealpha=0.5,ncol=10)

    plt.savefig(f'E:/Data/HEDP/plot_table/DREP_P_{data[0,0]:3.1e}.pdf',bbox_inches='tight',format='pdf')
    ...
    cl = 0
    for data in plot_list2:
        plt.plot(data[:,1],data[:,3],'o',mec=line_sty[cl],mfc='none',markersize=10,label = f'{data[0,0]:.2f}g/cc')
        cl+=1

    plt.ticklabel_format(style='sci', axis='both')#, scilimits=(0,0))    
    plt.tick_params(axis='both', labelsize=20)
    plt.xlabel('Temperature (K)',fontsize = 30)
    plt.ylabel('Internal Energy (eV/atom)',fontsize = 30)
    plt.xscale('log')
    plt.yscale('log')
    plt.title('Internal Energy',fontsize = 30)
    # plt.legend(loc=2,fontsize=10,fancybox=True, framealpha=0.5)
    plt.legend(bbox_to_anchor=(0.5, 1.2),loc='upper center',fontsize=20,fancybox=True, framealpha=0.5,ncol=10)

    plt.savefig(f'E:/Data/HEDP/plot_table/DREP_E_{data[0,0]:3.1e}.pdf',bbox_inches='tight',format='pdf')
    ...
    cl = 0
    for data in plot_list2:
        plt.plot(np.log10(data[:,1]),np.log10(data[:,3]+16),'o',mec=line_sty[cl],mfc='none',markersize=10,label = f'{data[0,0]:.2f}g/cc')
        cl+=1
    ...
    plt.savefig(f'E:/Data/HEDP/plot_table/DREP_E_log_{data[0,0]:3.1e}.pdf',bbox_inches='tight',format='pdf')
